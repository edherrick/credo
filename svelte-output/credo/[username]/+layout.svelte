...
<div class="credo-header">
	<div class="credo-header-inner">
		...
		{/if}
		<div class="credo-stats">
			{#if credo.agendas?.length}
				<span class="credo-stat">{credo.agendas.length} agenda{credo.agendas.length === 1 ? '' : 's'}</span>
			{/if}
			{#if credo.entities?.length}
				<span class="credo-stat">{credo.entities.length} entit{credo.entities.length === 1 ? 'y' : 'ies'}</span>
			{/if}
		</div>
	</div>
